/**
	...
	@methods
		enable(index,bool) // Включает и отключает по индексу кнопку 
		isEnable(index) // Возвращает значение bool, включена или отключена кнопку.
		enableAll(bool) // включает и отключает весь компонент
		isEnableAll() // возвращает bool, включен или тключен компонент
		changeMod(String,index) // String - режимы для переключения кнопки с указанным индексом. Режимы : normal, select, disable (все это описано в createButton3Ext)
	@return
**/
