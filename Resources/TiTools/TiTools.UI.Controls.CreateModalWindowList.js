...
function createModalWindowList(params)
{
	...
	for(var i = 0; i < params.list.length; i++)
	{
		...
		selectImage.hide();
		row.selectImage = selectImage;
		row.add(selectImage);
		row.addEventListener("click",function(event)
			{
				content.selectRow.selectImage.hide();
				content.selectRow = event.source;
				event.source.selectImage.show();
				content.data = event.source.data;
			}
		);
	}
	...
	rightButton.addEventListener("click",function(event)
		{
			Ti.App.fireEvent("modalListOk",{data : content.data});
		}
	);
